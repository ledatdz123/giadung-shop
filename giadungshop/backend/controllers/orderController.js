...
exports.updateStatusOrder = catchAsyncErrors(async (req, res, next) => {
    ...
    const message=`Giao thành công đơn hàng: ${order._id}.
 Địa chỉ: ${order.shippingInfo.address}.  
 SĐT: ${order.shippingInfo.phone}. 
 Tổng thành tiền: ${order.totalPrice}.`
    order.orderStatus = req.body.orderStatus
    if (req.body.orderStatus === 'Received') {
        await sendMail({
            email: user.email,
            subject: `Mail đơn hàng của bạn`,
            message,
        })
...
exports.hotProductBetween = catchAsyncErrors(async (req, res) => {
    ...
    if (dateFrom && dateTo) {
        ...
        var from = part1[2]+"-"+part1[1]+"-"+part1[0]
        var to = part2[2]+"-"+part2[1]+"-"+part2[0]
...
exports.totalBetweenDate = catchAsyncErrors(async (req, res) => {
    const { dateFrom, dateTo } = req.body
    if (dateFrom && dateTo) {
        let part1 = dateFrom.split('-')
        let part2 = dateTo.split('-')
